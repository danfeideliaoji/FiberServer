...
TcpServer::TcpServer(FiberServer::IOManager* worker,
                    FiberServer::IOManager* io_worker,
                    FiberServer ::IOManager* accept_worker)
    :m_worker(worker)
    ,m_ioWorker(io_worker)
    ,m_acceptWorker(accept_worker)
    ,m_recvTimeout(g_tcp_server_read_timeout->getValue())
    ,m_name("sylar/1.0.0")//还是默认用大佬的名字吧哈哈
    ,m_isStop(true) {
}
...
bool TcpServer::bind(const std::vector<Address::ptr>& addrs
    ...
    for(auto& addr : addrs) {
        ...
        if(!sock->bind(addr)) {
            FIBER_LOG_ERROR(g_logger) << "bind fail errno="
        ...
        if(!sock->listen()) {
            FIBER_LOG_ERROR(g_logger) << "listen fail errno="
                << errno << " errstr=" << strerror(errno)
                << " addr=[" << addr->toString() << "]";
            fails.push_back(addr);
            continue;
    ...
    for(auto& i : m_socks) {
        FIBER_LOG_INFO(g_logger) << "type=" << m_type
            << " name=" << m_name
            << " ssl=" << m_ssl
...
void TcpServer::handleClient(Socket::ptr client) {
    FIBER_LOG_INFO(g_logger) << "handleClient: " << *client;
}
