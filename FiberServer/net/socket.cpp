...
#include "FiberServer/fd_manager.h" 
...
void Socket::initSock() {
    int val = 1;
    setOption(SOL_SOCKET, SO_REUSEADDR, val);//运行端口复用
    if(m_type == SOCK_STREAM) {
        setOption(IPPROTO_TCP, TCP_NODELAY, val);//禁用 Nagle 算法
    }
}

void Socket::newSock() {
    m_sock = socket(m_family, m_type, m_protocol);//这里FI劫持后socket
    if(FIBER_LIKELY(m_sock != -1)) {
        initSock();
    } else {
        FIBER_LOG_ERROR(g_logger) << "socket(" << m_family
            << ", " << m_type << ", " << m_protocol << ") errno="
            << errno << " errstr=" << strerror(errno);
    }
}

Socket::ptr Socket::CreateTCP(FiberServer::Address::ptr address) {
    Socket::ptr sock(new Socket(address->getFamily(), TCP, 0));
    return sock;
}
Socket::ptr Socket::CreateUDP(FiberServer::Address::ptr address) {
...
bool Socket::getOption(int level, int option, void* result, socklen_t* len) {
    ...
    if(rt) {
        FIBER_LOG_DEBUG(g_logger) << "getOption sock=" << m_sock
...
bool Socket::setOption(int level, int option, const void* result, socklen_t len) {
    if(setsockopt(m_sock, level, option, result, (socklen_t)len)) {
        FIBER_LOG_DEBUG(g_logger) << "setOption sock=" << m_sock
            << " level=" << level << " option=" << option
            << " errno=" << errno << " errstr=" << strerror(errno);
        return false;
    }
...
Socket::ptr Socket::accept() {
    ...
    int newsock =::accept(m_sock, nullptr, nullptr);//调用劫持后的accept  
    if(newsock == -1) {
        FIBER_LOG_ERROR(g_logger) << "accept(" << m_sock << ") errno="
            << errno << " errstr=" << strerror(errno);
        return nullptr;
    }
...
bool Socket::bind(const Address::ptr addr) {
    //m_localAddress = addr;
    if(!isValid()) {//如果当前还没有建立socket
        newSock();
        if(FIBER_UNLIKELY(!isValid())) {
            return false;
        }
    }

    if(FIBER_UNLIKELY(addr->getFamily() != m_family)) {
        FIBER_LOG_ERROR(g_logger) << "bind sock.family("
    ...
    if(uaddr) {
        ...
        if(sock->connect(uaddr)) {//如果进程还占着
            return false;
        } else {//进程不存在了 有残留的文件
            FiberServer::FSUtil::Unlink(uaddr->getPath(), true);
        }
    }

    if(::bind(m_sock, addr->getAddr(), addr->getAddrLen())) {
        FIBER_LOG_ERROR(g_logger) << "bind error errrno=" << errno
...
bool Socket::reconnect(uint64_t timeout_ms) {
    if(!m_remoteAddress) {
        FIBER_LOG_ERROR(g_logger) << "reconnect m_remoteAddress is null";
        return false;
    }
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms) {
    ...
    if(FIBER_UNLIKELY(addr->getFamily() != m_family)) {
        FIBER_LOG_ERROR(g_logger) << "connect sock.family("
            << m_family << ") addr.family(" << addr->getFamily()
            << ") not equal, addr=" << addr->toString();
        return false;
    }
    ...
    if(timeout_ms == (uint64_t)-1) {
        if(::connect(m_sock, addr->getAddr(), addr->getAddrLen())) {
            FIBER_LOG_ERROR(g_logger) << "sock=" << m_sock << " connect(" << addr->toString()
...
bool Socket::listen(int backlog) {
    if(!isValid()) {
        FIBER_LOG_ERROR(g_logger) << "listen error sock=-1";
        return false;
    }
    if(::listen(m_sock, backlog)) {
        FIBER_LOG_ERROR(g_logger) << "listen error errno=" << errno
            << " errstr=" << strerror(errno);
        return false;
    }
    return true;
...
Address::ptr Socket::getLocalAddress() {
    ...
    if(getsockname(m_sock, result->getAddr(), &addrlen)) {
        FIBER_LOG_ERROR(g_logger) << "getsockname error sock=" << m_sock
            << " errno=" << errno << " errstr=" << strerror(errno);
        return Address::ptr(new UnknownAddress(m_family));
    }
    if(m_family == AF_UNIX) {
...
bool Socket::cancelRead() {
    return IOManager::GetThis()->cancelEvent(m_sock, FiberServer::IOManager::READ);
}

bool Socket::cancelWrite() {
    return IOManager::GetThis()->cancelEvent(m_sock, FiberServer::IOManager::WRITE);
}
...
bool SSLSocket::connect(const Address::ptr addr, uint64_t timeout_ms) {
    ...
    if(v) {
        ...
        v = (SSL_connect(m_ssl.get()) == 1);//进行ssl握手
    }
    return v;
}
...
bool SSLSocket::loadCertificates(const std::string& cert_file, const std::string& key_file) {
    ...
    if(SSL_CTX_use_certificate_chain_file(m_ctx.get(), cert_file.c_str()) != 1) {
        FIBER_LOG_ERROR(g_logger) << "SSL_CTX_use_certificate_chain_file("
            << cert_file << ") error";
        return false;
    }
    if(SSL_CTX_use_PrivateKey_file(m_ctx.get(), key_file.c_str(), SSL_FILETYPE_PEM) != 1) {
        FIBER_LOG_ERROR(g_logger) << "SSL_CTX_use_PrivateKey_file("
            << key_file << ") error";
        return false;
    }
    if(SSL_CTX_check_private_key(m_ctx.get()) != 1) {
        FIBER_LOG_ERROR(g_logger) << "SSL_CTX_check_private_key cert_file="
