...
public:
     ...
     */
    static std::shared_ptr<IPadress> LookupAnyIPAdress(const std::string& host,
    int family=AF_INET,int type = 0,int protocol=0);
     ...
     * @return 是否获取成功
     */
    static bool GetInterfaceAddresses(std::multimap<std::string,std::pair<Address::ptr,uint32_t>>& results,
    int family=AF_INET);
     ...
     * @return 是否获取成功
     */
    static bool GetInterfaceAddresses(std::vector<std::pair<Address::ptr,uint32_t>>
    &results,const std::string& iface,int family=AF_INET);
    ...
    //后面的const也能形成重载 这里的const修饰this的
