...
#include "FiberServer/base/nocopyable.h"
#include "address.h"
...
public:
    ...
    /**
     * @brief 获取sockopt模板 @see getsockopt 这里result千万不能传指针注意！！！！
     */
    template<class T>
...
public:
    ...
    static SSLSocket::ptr CreateTCP(FiberServer::Address::ptr address);
    ...
    //下面俩是用于UDP的 SSl是针对的TCP的 所有如果有调用下面的直接断言错误
...
private:
    std::shared_ptr<SSL_CTX> m_ctx;//存放ssl的证书和私钥
