...
namespace RPC
{
    ...
    public:
        ...
        { // 将配置格式转为YAML的string格式
            ...
            {
                RPC_LOG_ERROR(RPC_LOG_ROOT()) << "ConfigVar::toString exception "
        ...
        { // 读取YAML的string
            ...
            {
                RPC_LOG_ERROR(RPC_LOG_ROOT()) << "ConfigVar::fromString exception "
...
class Config
{
    public:
        ...
        typedef RWMutex RWMutexType;
        // 使用静态的原因是先初始化防止用时还没有 因为如果创建全局变量的话 顺序不确定 用静态防止其他全局变量用时表还没创建
            ...
            const std::string &descripton = "")
        {
            auto map = GetDatas(); // 获得静态初始化表
            {
                ...
                { // 找到
                    ...
                    if (tmp)
                    { // 转换成功
                        RPC_LOG_INFO(RPC_LOG_ROOT()) << "Lookup name=" << name << " exists";
                        return tmp;
                    }
                    else
                    { // 转换失败
                        RPC_LOG_ERROR(RPC_LOG_ROOT()) << "Lookup name=" << name << " exists but type not "
            ...
            { // 判断命名是否符合规则
                RPC_LOG_ERROR(RPC_LOG_ROOT()) << "Lookup name invalid " << name;
