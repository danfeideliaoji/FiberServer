...
#define RPC_LOG_LEVEL(logger,level)\
    if(logger->getLevel()<=level) \
        RPC::LogEventWrap(RPC::LogEvent::ptr(new RPC::LogEvent(logger,level,\
        __FILE__,__LINE__,0, RPC::GetThreadId(),-1,time(0),"unknown"))).getSS()
//GetThreadId()在util里
//各种等级的宏
#define RPC_LOG_DEBUG(logger) RPC_LOG_LEVEL(logger, RPC::LogLevel::Level::DEBUG)
#define RPC_LOG_INFO(logger) RPC_LOG_LEVEL(logger, RPC::LogLevel::Level::INFO)
#define RPC_LOG_WARN(logger) RPC_LOG_LEVEL(logger, RPC::LogLevel::Level::WARN)
#define RPC_LOG_ERROR(logger) RPC_LOG_LEVEL(logger, RPC::LogLevel::Level::ERROR)
#define RPC_LOG_FATAL(logger) RPC_LOG_LEVEL(logger, RPC::LogLevel::Level::FATAL)
#define RPC_LOG_FMT_LEVEL(logger, level, fmt, ...) \
    if(logger->getLevel() <= level) \
        RPC::LogEventWrap(RPC::LogEvent::ptr(new RPC::LogEvent(logger, level, \
                        __FILE__, __LINE__, 0, RPC::GetThreadId(),\
                -1, time(0), "unknown"))).getEvent()->format(fmt,##__VA_ARGS__)
#define RPC_LOG_FMT_DEBUG(logger, fmt, ...) RPC_LOG_FMT_LEVEL(logger, RPC::LogLevel::Level::DEBUG, fmt,##__VA_ARGS__)                
#define RPC_LOG_FMT_INFO(logger,fmt,...) RPC_LOG_FMT_LEVEL(logger,RPC::LOGLEVEL::Level::INFO,fmt,##__VA_ARGS__) 
#define RPC_LOG_FMT_WARN(logger,fmt,...) RPC_LOG_FMT_LEVEL(logger,RPC::LOGLEVEL::Level::WARN,fmt,##__VA_ARGS__)
#define RPC_LOG_FMT_ERROR(logger,fmt,...) RPC_LOG_FMT_LEVEL(logger,RPC::LOGLEVEL::Level::ERROR,fmt,##__VA_ARGS__)
#define RPC_LOG_FMT_FATAL(logger,fmt,...) RPC_LOG_FMT_LEVEL(logger,RPC::LOGLEVEL::Level::FATAL,fmt,##__VA_ARGS__)
//获取主日志器
#define RPC_LOG_ROOT() RPC::LoggerMgr::GetInstance()->getRoot()
//获取指定名称的日志器 如果没有则创建
#define RPC_LOG_NAME(name) RPC::LoggerMgr::GetInstance()->getLogger(name)
namespace RPC{
class Logger;//前向声明
class LogLevel{
public:
