...
#include "config.h"
namespace RPC{
static RPC::Logger::ptr g_logger=RPC_LOG_NAME("system");
//查找获得对应的基类
...
const YAML::Node& node,std::list<std::pair<std::string,const YAML::Node>>& output)
{
    ...
    if(prefix.find_first_not_of("abcdefghikjlmnopqrstuvwxyz._012345678")!=std::string::npos){
        RPC_LOG_ERROR(RPC_LOG_ROOT())<<"Config invalid name: "<<prefix<<" : "<<node;
        return;
    }
...
void Config::LoadFromYaml(const YAML::Node &root){
    ...
    for(auto &i:all_nodes){
        ...
        if(var) {//将对应的值转为字符串
            RPC_LOG_DEBUG(g_logger)<<"config "<<key;
...
void Config::LoadFromConfDir(const std::string& path,bool force){
    ...
    try{
        ...
        LoadFromYaml(root);
        RPC_LOG_INFO(g_logger)<<"LoadConfFile file="<<path<<" ok";
    }
    catch(...){
        RPC_LOG_ERROR(g_logger)<<"LoadConfFile file="<<path<<" failed";
    }
