...
#undef XX
    ...
    {
        char *buf = nullptr;
        int len = vasprintf(&buf, fmt, al); // 可为buf自动分配内存
        if (len != -1)
        {
    ...
    void LogFormatter::init()
    {
        ...
        */
        ...
        for (size_t i = 0; i < m_pattern.size(); ++i)
        {
            ...
            {
                vec.emplace_back("%", "", 0); // 这里选择将%当作普通字符处理
                continue;                     // 不用i=n 因为循环结束 i++到这个非法字符
            }
            std::string key(1, m_pattern[n]);
...
#undef XX
        ...
        for (auto &i : vec)
        {
            ...
            else if (std::get<2>(i) == 1)
            {
                ...
                m_items.emplace_back(s_format_items[std::get<0>(i)](std::get<1>(i)));
            }
        }
    }
    ...
    struct LogIniter
    {
        ...
        { // 添加监听器 当值修改时触发
            g_log_defines->addListener([](const std::set<LogDefine> &old_value, const std::set<LogDefine> &new_value)
                                       {
            RPC_LOG_INFO(RPC_LOG_ROOT())<<"on_logger_conf_changed";
            ...
            for(auto &i:new_value)
            {
                ...
                for(auto& a : i.appenders) {//读取LogAppenderDefine
                    LogAppender::ptr ap;//给logappender对应值
                    if(a.type == 1) {//1表文件
                        ap.reset(new FileLogAppender(a.file));
                    } 
                    else if(a.type == 2){//2表示std
                        //  if(!RPC::EnvMgr::GetInstance()->has("d")) 后续升级 
            ...
            //删除在新配置不存在的配置 
             for(auto& i : old_value) {
                ...
                if(it == new_value.end()) {
                    ...
                    auto logger = RPC_LOG_NAME(i.name);
