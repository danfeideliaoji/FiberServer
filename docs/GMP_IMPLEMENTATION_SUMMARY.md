...
- HTTP server、servlet 路由、制品预检、直传/分片上传、列表、下载和删除等业务接口。
...
```text
基于 C++ 协程调度器的高并发 CI/CD 构建制品仓库：借鉴 Go GMP 思想实现 M:N 协程调度、epoll 事件驱动和 work stealing。
```
...
以下业务压测结论来自旧普通文件公开接口仍存在时的历史结果。当前旧公开接口和对应脚本已经删除，仅保留为性能边界参考：
...
这些不是当前轻量制品仓库展示版本必须实现的内容。当前版本的重点是让项目具备一个可讲清楚、可运行、可观测、可压测的 M:N 协程调度模型。
